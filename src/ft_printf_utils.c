...
/*   Updated: 2022/09/25 03:23:42 by ttakami          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	ft_put_sth(int c, va_list args)
{
	...
	if (c == 'c')
		...
		len = ft_put_string(va_arg(args, char *));
	else if (c == 'p')
		len = ft_put_point((uintptr_t)va_arg(args, void *), "0123456789abcdef");
	else if (c == 'd' || c == 'i')
		len = ft_put_int(va_arg(args, int));
	else if (c == 'u')
		len = ft_put_uint_base(va_arg(args, unsigned int), "0123456789");
	else if (c == 'x')
		len = ft_put_uint_base(va_arg(args, unsigned int), "0123456789abcdef");
	else if (c == 'X')
		len = ft_put_uint_base(va_arg(args, unsigned int), "0123456789ABCDEF");
	else if (c == '%')
		len = ft_put_char(c);
